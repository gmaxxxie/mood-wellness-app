...
start_databases() {
    ...
    if [ -f "docker-compose.yml" ]; then
        ...
        # 验证连接
        echo "验证PostgreSQL连接..."
        docker compose exec -T postgres pg_isready -U admin -d mood_wellness
...
init_database() {
    ...
    if [ -f "docker-compose.yml" ]; then
        # 使用Docker执行迁移
        docker compose exec -T postgres psql -U admin -d mood_wellness < ../database/init.sql
        docker compose exec -T postgres psql -U admin -d mood_wellness < ../database/seed_psychology_data.sql
        docker compose exec -T postgres psql -U admin -d mood_wellness < ../database/seed_solutions.sql
    else
        # 直接执行SQL文件
